...
template sudoku() {
    ...
    component inRange[9][9][2];
    
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            for (var k=0; k<2; k++) {
                inRange[i][j][k] = rangeproof(32);
                inRange[i][j][k].range[0] <== 0;
                inRange[i][j][k].range[1] <== 9;
                if (k==0) {
                    inRange[i][j][k].in <== puzzle[i][j];
                } else {
                    inRange[i][j][k].in <== solution[i][j];
                }
                assert(inRange[i][j][k].out);
            }
